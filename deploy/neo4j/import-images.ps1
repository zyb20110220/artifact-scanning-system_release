# ============================================================
# import-images.ps1 —— 将 Neo4j 镜像导入 k3d 集群节点（离线化）
# 用法：.\import-images.ps1 [-Force]
#       镜像来源：经代理 docker pull（docker.io/neo4j 社区版）
#       幂等：节点已具备镜像时自动跳过。
# ============================================================
param([switch]$Force)
...
if (-not $Force) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [已有]" -ForegroundColor DarkGray
    } else {
        docker pull $img
    }
...
$tar = Join-Path $env:TEMP "neo4j-imgs\neo4j.tar"
...
foreach ($node in $nodes) {
    ...
    docker cp "$tar" "${node}:/tmp/neo4j-import.tar"
    docker exec $node ctr -n k8s.io images import /tmp/neo4j-import.tar
    docker exec $node rm -f /tmp/neo4j-import.tar 2>$null
}
...
Write-Host "[完成] Neo4j 镜像已就绪。" -ForegroundColor Green
