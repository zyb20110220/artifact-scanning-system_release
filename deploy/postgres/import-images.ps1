# ============================================================
# import-images.ps1 —— 将 PostgreSQL 镜像导入 k3d 节点
# 幂等：节点已具备则跳过（-Force 强制）；逐镜像导入（规避合并 tar bug）
# ============================================================
param(
    [switch]$Force
)
...
# PostgreSQL 镜像（与 values.yaml 对齐；latest = 18.6.0）
$images = @(
...
# ------------------------------------------------------------
# 第 2 步：逐镜像 save 到临时目录
# ------------------------------------------------------------
Write-Host "[2/3] 逐镜像 docker save ..." -ForegroundColor Cyan
$imgDir = Join-Path $env:TEMP "postgres-imgs"
...
foreach ($node in $nodes) {
    ...
    foreach ($img in $images) {
        ...
        docker cp "$($tarMap[$img])" "${node}:/tmp/pg-import.tar"
        if ($LASTEXITCODE -ne 0) {
            Write-Host "  [错误] docker cp 失败：$node" -ForegroundColor Red
            exit 1
        }
        docker exec $node ctr -n k8s.io images import /tmp/pg-import.tar
        if ($LASTEXITCODE -ne 0) {
            Write-Host "  [错误] ctr import 失败：$node" -ForegroundColor Red
            exit 1
        }
        docker exec $node rm -f /tmp/pg-import.tar 2>$null
    }
}

Write-Host ""
Write-Host "[完成] PostgreSQL 镜像已就绪。可运行 .\postgres\install.ps1 部署。" -ForegroundColor Green
