...
| 5.7 | LLM 输出质量评估（考古专家盲评） | ✅ | 2026-08-27 | 自动化评估脚本+盲评评分卡；基线样本：3B 基座文化/年代全 0（印证需微调） |
...
- [x] 2026-08-27：LLM 输出质量评估（阶段 5.7）
  - 交付：src/artifact_scan/llm_eval.py（自动化评估）+ docs/llm-quality-eval.md（指标说明 + 专家盲评评分卡）
  - 自动化流程：采样带图文物 → 调用 Ollama(qwen2.5-vl:3b) 按提示词返回紧凑 JSON（period/culture/confidence）→ 与标注真值比对 → 输出精确/包含匹配准确率 + 逐条 JSON 报告
  - 基线实测（n=5）：culture_exact=0.0 / period_exact=0.0（含真值过滤后）。模型能正确输出 JSON，但把文化判成风格（法国→印象派、意大利→欧洲/罗马）、英文用词（佛兰德斯→Netherlandish），年代粗粒度（近代→19世纪）——**未微调 3B 模型的真实薄弱点，印证 5.6 LoRA 微调必要性**
  - 验证（2026-08-27）：逐条核查确认模型 5/5 均产出有效结构化 JSON（含 markdown 围栏也能用首末 `{}` 抽取），独立重算指标一致；0.0 是**严格 exact/contains 字符串匹配**结果——模型答案多为**语义相邻**（印象派之于法国印象派画、Netherlandish 之于佛兰德斯、罗马之于意大利），但未命中标签原文，故部分关联未计入；需结合专家盲评判断可接受度
  - 指标修正：ground-truth 为空的条目不计入对应指标（避免缺失真值被误判为错误）
  - 专家盲评：评分卡（年代/文化/依据/术语/结构/整体 6 维 1-5 分 + 是否认可结论），供考古专家对模型输出打散后盲评
  - 说明：属质量评估框架 + 基座基线；待 5.6 微调产物产出后，用同一批测试集（--ids）对比基座 vs 微调模型
</details>
...
| 6.3 | 检索结果画廊 + 详情面板 | ✅ | 2026-08-27 | 相似文物网格画廊（缩略图+得分+来源），点击打开详情弹窗（大图/时期/文化/描述/馆藏链接） |
| 6.4 | 证据链交互式网络图 | ✅ | 2026-08-27 | **原生 SVG 实现**（拖拽节点/平移/滚轮缩放/悬停高亮+提示/双击复位），离线免 D3/CDN |
| 6.5 | 断代报告渲染（结构化 JSON → 可视化） | ✅ | 2026-08-27 | 结论卡（年代/文化/置信度）+ LLM 分析 + 相似列表 |
| 6.6 | 时间轴 / 地图可视化 | ✅ | 2026-08-27 | 朝代时间轴（待鉴定+相似文物时期标记）+ 文化/时期/材料/器型/纹饰要素分布 |
| 6.7 | 移动端适配 | ⬜ | | |
...
- [x] 2026-08-27：证据链图谱重构（美化 + 完善）
  ...
  - **6.3 / 6.4 / 6.6（同日）**：
    - 后端 `_milvus_recall` 扩展：similar 附 `image_url/description/period/culture/url`（读取 annotated 元数据）
    - 6.3 画廊：相似文物网格卡片（缩略图+得分+来源，图加载失败回退占位），点击打开详情弹窗（大图/时期/文化/来源/相似度/描述/馆藏链接）；Esc/遮罩/× 关闭
    - 6.4 交互图：原生 SVG 网络图——节点拖拽、背景平移、滚轮缩放、悬停高亮相连边+工具提示、双击复位（免 D3/CDN，离线可用）
    - 6.6 时间轴：朝代标尺（新石器→近代）上标记待鉴定文物（印泥红）与相似文物（青铜）时期；文化分布：culture/period/material/form/decoration 分色要素 chips
    - 验证：模拟数据全渲染通过（画廊 3 卡/交互图 10 节点/时间轴 4 点/文化 6 chips/弹窗详情）；滚轮缩放 scale 1→1.12；v=12
  - **布局调整（同日）**：相似文物画廊横向展开整行（flex + 横向滚动）；时间轴移入其下并移除时期标尺（仅标签+图例）；证据链图谱移至最底部全宽；v=13
  - **Top-10 + 时间轴兜底（同日）**：`_milvus_recall` topk 5→10；时间轴不再因无法映射年份显示"暂无时期数据"——未知时期统一归入右侧"未定 ×N"标签并加图例；v=14
  - **图谱静态化 + 随机排布（同日）**：移除节点拖拽/平移/滚轮缩放/双击复位（图谱固定不变），仅保留悬停高亮+提示；节点布局加角度/半径随机抖动打破规整圆环；提示文案改"悬停查看详情"；v=15
  - **图谱节点重叠修复（同日）**：随机排布导致模块相交 → 增加碰撞避免（按节点尺寸对重叠节点做轻量推开、中心节点固定、坐标夹取画布内）；顺带修复碰撞代码作用域 bug（`a is not defined`）；验证 19 节点 0 重叠；v=16
</details>
